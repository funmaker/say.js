...
class SayPlatformBase {
  ...
  speak (text, voice, speed) {
    return new Promise((res, rej) => {
      if (!text) {
        rej(new TypeError('say.speak(): must provide text parameter'))
      }
      ...
      this.child.addListener('exit', (code, signal) => {
        if (code === null || signal !== null) {
          return rej(new Error(`say.speak(): could not talk, had an error [code: ${code}] [signal: ${signal}]`))
        }
   ...
   */
  export (text, voice, speed, filename) {
    return new Promise((res, rej) => {
      if (!text) {
        rej(new TypeError('say.export(): must provide text parameter'))
      }
  
      if (!filename) {
        rej(new TypeError('say.export(): must provide filename parameter'))
      }
      ...
      this.child.addListener('exit', (code, signal) => {
        if (code === null || signal !== null) {
          return rej(new Error(`say.export(): could not talk, had an error [code: ${code}] [signal: ${signal}]`))
        }
  ...
  stop () {
    return new Promise((res, rej) => {
      if(!this.child) {
        return setImmediate(() => {
          rej(new Error('say.stop(): no speech to kill'))
        })
      }
  ...
  getInstalledVoices () {
    return new Promise((res, rej) => {
      ...
      this.child.addListener('exit', (code, signal) => {
        if (code === null || signal !== null) {
          return rej(new Error(`say.getInstalledVoices(): could not get installed voices, had an error [code: ${code}] [signal: ${signal}]`))
        }
        if (voices.length > 0) {
          voices = voices.split('\r\n')
